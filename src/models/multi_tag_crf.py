...
class CRF(nn.Module):
    ...
    def __init__(self, num_tags: int, batch_first: bool = False, constrain_every: bool = False, allowed_transitions: List[Tuple[int, int]] = None, allowed_start: List[int] = None, allowed_end: List[int] = None, include_start_end_transitions: bool = False, init_weight: float = 0.577350) -> None:
        if num_tags <= 0:
        ...
        if self.include_start_end_transitions:
            ...
            #self._constraint_start_mask = torch.nn.Parameter(constraint_start_mask, requires_grad=False)
            #self._constraint_end_mask = torch.nn.Parameter(constraint_end_mask, requires_grad=False)
            self.register_buffer('_constraint_start_mask', constraint_start_mask)
            self.register_buffer('_constraint_end_mask', constraint_end_mask)
    ...
    def _compute_seq_score( # input likelihood
        ...
        for i in range(1, seq_length):
            ...
            # shape: (batch_size,)
            score += emissions[i, torch.arange(batch_size), tags[i]] * mask[i] # TODO unary score
    ...
    def _topk_viterbi_decode(self, emissions: torch.FloatTensor,
                        mask: torch.ByteTensor, top_k: int = 5)-> List[List[List[int]]]:
        ...
        for i in range(mask.shape[1]):
            paths_i, scores_i = self._topk_viterbi_decode_sample(emissions[:,i,:], mask[:,i], top_k)
        ...
        for idx, s in enumerate(scores):
            llhs.append((s-log_denominator[idx]).detach().cpu().numpy())
        #likelihoods = [torch.exp(x-log_denominator) for x in scores] #x: (n_paths)
    ...
    def _topk_viterbi_decode_sample(self, emissions: torch.FloatTensor,
                        mask: torch.ByteTensor, top_k: int = 5):
        '''Return the top k viterbi paths for an input. NOTE this does not operate on batches.'''
        # follows https://gist.github.com/PetrochukM/afaa3613a99a8e7213d2efdd02ae4762
        ...
        if self.include_start_end_transitions:
            potentials_0 = tag_sequence[0, :] + self.start_transitions
            path_scores.append(potentials_0.unsqueeze(0))
        else:
        ...
        for timestep in range(1, sequence_length):
            ...
            if timestep == sequence_length-1 and self.include_start_end_transitions:
        ...
        for i in range(max_k):
            viterbi_path = [best_paths[i].item()] # TODO this is torhc.Tensor - need or not?
    ...
    def _compute_log_alpha(self,
        ...
        for i in seq_iterator:
            ...
            log_prob.append(score * mask[i].unsqueeze(1) + log_prob[-1] * (1.-mask[i]).unsqueeze(1))
